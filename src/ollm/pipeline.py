...
class LayerPipeline:
    ...
    def __init__(self, loader, stats=None, num_layers=32):
        ...
        # FIX: Separate state tracking for CPU and GPU buffers. This is the core of the fix.
        # `cpu_buffer_content` tracks what's loaded from disk into a CPU buffer.
        # `gpu_buffer_content` tracks what has been successfully transferred to a GPU buffer.
    ...
    def _disk_loader_worker(self):
        ...
        while not self.stop_thread.is_set():
            try:
                ...
                # FIX: Update the CPU buffer's content tag.
    ...
    def _transfer_cpu_to_gpu(self, buffer_idx: int):
        ...
        # FIX: Atomically update the GPU buffer's content tag. This happens *after* the copy
        # is enqueued on the stream, so it correctly reflects the future state of the buffer.
        self.gpu_buffer_content[buffer_idx] = self.cpu_buffer_content.get(buffer_idx, (-1, -1))
        
        if self.stats:
            self.stats.set("cpu_to_gpu", t1)
        
        layer_idx, generation = self.gpu_buffer_content.get(buffer_idx, (-1, -1))
        gen_str = f"[gen{generation}]" if generation > 0 else ""
    ...
    def initialize_pipeline(self, start_layer: int = 0):
        ...
        for i in range(2):
            ...
            if layer_idx < self.num_layers:
                ...
                self.cpu_buffer_content[i] = (layer_idx, 0)
                self._transfer_cpu_to_gpu(i) # This will also set gpu_buffer_content
                print(f"[Pipeline] Layer {layer_idx} pre-loaded to GPU buffer {i}")
        
        self.transfer_stream.synchronize()
        
        if start_layer + 2 < self.num_layers:
            self._schedule_disk_load(start_layer + 2, 0, 0)
        if start_layer + 3 < self.num_layers:
            self._schedule_disk_load(start_layer + 3, 0, 1)
    ...
    def get_layer_and_advance(self, layer_idx: int) -> Dict[str, torch.Tensor]:
        ...
        # The main compute stream must wait for the transfer of the *current* layer to be complete.
        ...
        # FIX: Check the GPU buffer's content, not the CPU buffer's.
        if self.gpu_buffer_content.get(current_buffer_idx) != expected_key:
            print(f"[Pipeline] WARNING: Buffer {current_buffer_idx} mismatch! Expected {expected_key}, found {self.gpu_buffer_content.get(current_buffer_idx)}. Sync loading.")
            base = f"model.layers.{layer_idx}."
            sync_dict = self.loader.load_dict_to_cuda(base)
            for k, t in sync_dict.items(): self.gpu_buffers[current_buffer_idx][k].copy_(t)
            self.gpu_buffer_content[current_buffer_idx] = expected_key
    ...
    def reset_generation_counter(self):
        ...
        self._layers_processed = 0
        self.compute_buffer_idx = 0
        # FIX: Reset all state dictionaries
