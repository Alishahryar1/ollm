...
class InferencePipelined:
	...
	def hf_download(self, model_dir):
		...
		urls = {
			"llama3-1B-chat": "unsloth/Llama-3.2-1B-Instruct",
	...
	def ini_model(self, models_dir="./models/", force_download=False):
		...
		# Special handling for pipelined Llama models
		if self.model_id in ["llama3-1B-chat", "llama3-3B-chat", "llama3-8B-chat"] and self.use_pipeline:
			from . import llama_pipelined
			llama_pipelined.loader = SingleDenseWeightsLoader(model_dir) if self.model_id in ["llama3-1B-chat"] else DenseWeightsLoader(model_dir)
			llama_pipelined.stats = self.stats
			self.model = llama_pipelined.MyLlamaForCausalLM.from_pretrained(
				model_dir, 
			...
			# Fallback to original llama.py for non-pipelined
			from . import llama
			llama.loader = SingleDenseWeightsLoader(model_dir) if self.model_id in ["llama3-1B-chat"] else DenseWeightsLoader(model_dir)
	...
	def offload_layers_to_cpu(self, **args):
		...
		if hasattr(self.model, 'offload_layers_to_cpu'):
			...
			print("[Warning] offload_layers_to_cpu not available with pipelined model")
	...
	def offload_layers_to_gpu_cpu(self, **args):
		...
		if hasattr(self.model, 'offload_layers_to_gpu_cpu'):
			...
			print("[Warning] offload_layers_to_gpu_cpu not available with pipelined model")
